...
const GITHUB_USERNAME = 'govin002';
const CACHE_KEY = 'github_projects_cache';
const CACHE_DURATION = 3600000; // 1 hour

// Obfuscated Formspree ID
...
function getFallbackProjects() {
  return [
    ...
    },
    {
      ...
      desc: "A modern, user-friendly frontend for managing hospital operations and patient data.",
      tags: ["React", "JavaScript", "UI/UX"],
      status: "COMPLETED",
      link: "https://github.com/govin002/Hospital-Management-System",
      stars: 0,
      forks: 0,
      language: "JavaScript"
    },
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        console.warn('GitHub API rate limit hit, using fallback data');
    ...
    // Transform GitHub data to match project structure
    const transformedProjects = repos.map(repo => {
      ...
      const tags = repo.topics && repo.topics.length > 0 
        ? repo.topics.slice(0, 3).map(t => t.charAt(0).toUpperCase() + t.slice(1))
...
// Section Component with fixed spacing
function Section({ id, children, className = "" }) {
  return (
    <section id={id} className={`min-h-screen px-6 py-16 md:px-16 lg:px-32 ${className}`}>
      {children}
    </section>
  );
}
...
// Hero Component with fixed spacing
  ...
  return (
    <Section id="home" className="flex flex-col justify-center items-center text-center">
      <div className="max-w-2xl">
        ...
        <h1 className="font-mono text-4xl md:text-6xl font-bold mt-4 mb-4 tracking-tighter"
          style={{ color: PHOSPHOR, textShadow: `0 0 15px ${PHOSPHOR}33` }}>
          Govinda Prasad Shrestha
        </h1>
        <div className="font-mono text-sm md:text-base mb-8" style={{ color: "#888" }}>
...
// About Component with fixed spacing
function About() {
  return (
    <Section id="about">
      <Prompt text="cat about.txt" color={AMBER} />
      <div className="mt-4 border-l-2 pl-6 max-w-2xl" style={{ borderColor: PHOSPHOR }}>
...
// Project Card Component with GitHub stats
function ProjectCard({ title, desc, tags, status, link, stars, forks, language, homepage }) {
  const statusColor = status === "ACTIVE" ? PHOSPHOR : 
                     status === "COMPLETED" ? AMBER : 
                     status === "ARCHIVED" ? "#888" : 
                     status === "MAINTAINED" ? "#4CAF50" : "#aaa";
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <a href={link} target="_blank" rel="noopener noreferrer"
      ...
      <div className="flex justify-between items-start mb-3">
        <h3 className="font-mono font-bold text-base" style={{ color: hovered ? PHOSPHOR : "#eee" }}>
          {title}
        </h3>
        <div className="flex flex-wrap gap-2 items-center ml-2 shrink-0">
          {language && (
            <span className="font-mono text-[9px] px-2 py-1 border hidden sm:inline-block"
                  style={{ borderColor: "#39FF1420", color: "#888" }}>
              {language}
            </span>
          )}
          <span className="font-mono text-[10px] px-2 py-1 border uppercase tracking-tighter"
      ...
      <p className="font-mono text-[12px] leading-relaxed mb-4" style={{ color: hovered ? "#eee" : "#aaa" }}>
        {desc}
      </p>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {tags.map(t => (
          <span key={t} className="font-mono text-[11px] px-2 py-0.5 border"
            style={{ borderColor: AMBER + "40", color: AMBER, background: AMBER + "10" }}>
            {t}
          </span>
        ))}
      </div>
      ...
      <p className="font-mono text-[11px] mt-4 opacity-60 group-hover:opacity-100 transition-opacity" 
...
// Projects Component with dynamic fetching
    ...
    return (
      <Section id="projects">
        ...
        <div className="grid md:grid-cols-2 gap-5 max-w-4xl mx-auto">
          {[1,2,3,4].map(i => (
            <div key={i} className="border-2 p-5 animate-pulse" style={{ borderColor: "#39FF1420", background: "#181818" }}>
  ...
  if (error) {
    return (
      <Section id="projects">
        <Prompt text="git log --oneline projects/" color={AMBER} />
        <h2 className="font-mono text-3xl font-bold mt-4 mb-8" style={{ color: PHOSPHOR }}>Projects</h2>
        <div className="text-center p-8 border-2 max-w-2xl mx-auto" style={{ borderColor: "#ff4444", background: "#181818" }}>
          <p className="font-mono" style={{ color: "#ff4444" }}>[ERROR] {error}</p>
          <p className="font-mono text-sm mt-2" style={{ color: "#888" }}>Please try refreshing the page</p>
          <button 
            onClick={() => window.location.reload()} 
            className="font-mono text-sm mt-4 px-4 py-2 border"
    ...
    <Section id="projects">
      ...
      <div ref={listRef} className="grid md:grid-cols-2 gap-5 max-w-4xl mx-auto">
        {projects.length > 0 ? (
          ...
          <div className="col-span-2 text-center p-8" style={{ color: "#888" }}>
      ...
      <div className="text-center mt-10">
        <a href="https://github.com/govin002?tab=repositories" target="_blank" rel="noopener noreferrer"
          ...
          {`>> view all ${projects.length}+ repos on GitHub`}
        </a>
      </div>
    </Section>
  );
